...
const Dashboard = () => {
    ...
    useEffect(() => {
        const propietarioLocalStorage = localStorage.getItem('Usuario');
        const token = localStorage.getItem('token');
        setUsuario(propietarioLocalStorage === 'true');
        if (!token) {
                ...
                <ul className="mt-5 flex flex-col space-y-4">
                    <li className="text-center">
                        <div className="text-white bg-blue-800 px-3 py-2 rounded-md text-xl hover:bg-blue-700">
                            <Link to={Usuario ? "/dashboard/administrartienda" : "/dashboard/confirmacion"}>
                            {Usuario ? "Administrar tienda" : "Registrar tienda"}
                            </Link>
                        </div>
                    </li>
                </ul>
